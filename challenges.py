...
def displayData():
    ...
    if request.method == 'POST':
        ...
        value = request.form["ingredient"]
        api = requests.get(base_url, params = {"i": value, "format":"json"})
        text = json.loads(api.text)
        # if "results" in text:
        #   recipes = []
        #   for dic in text["results"]:
        #     # if dic["title"] not in recipes:
        #     recipes.append(dic["ingredients"])
        # return recipes
        return api.text
    else:
